...
export async function POST(req: NextRequest) {
  try {
    ...
    const { form, field, action, valueMasked, sessionId } = body || {};

    const { error } = await supabase.from('form_field_events').insert({
      form: typeof form === 'string' ? form : null,
      field: typeof field === 'string' ? field : null,
      action: typeof action === 'string' ? action : null,
      value_masked: typeof valueMasked === 'string' ? valueMasked : null,
